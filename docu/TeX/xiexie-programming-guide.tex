...
\begin{itemize}
	...
	\item \textbf{\texttt{MyModule.dll}} Shared library which runs the module code.
...
XVM_EXPORT XVM_INVOCATION_PROC xx_module_fetch_proc(int index) {
    return index < NUM_PROCS ? procList[index].proc : NULL;
}

XVM_EXPORT const char* xx_module_fetch_ident(int index) {
    return index < NUM_PROCS ? procList[index].ident : NULL;
}
\end{lstlisting}
