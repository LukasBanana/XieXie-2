...
Named parameters, like in \csharp, are supported as well. They also have the same rules as in \csharp,
i.e. if you started to write named parameters in a procedure call, you need to finish it till the last argument.
...
class Widget {
    ...
    void call() {
        ...
        f(1, y: 2, 3) // Error, can not start with named parameters and continue with unnamed
    }
}
\end{lstlisting}
But what are named parameters good for? Consider the following function signature:
\begin{lstlisting}[numbers=left]
// User account class.
class UserAccount {
    init(
...
\begin{itemize}
	...
	\item \textbf{\texttt{export}}, \textbf{\texttt{export(String label)}}
		Exports the address of a \textit{static procedure}.
	\item \textbf{\texttt{bind}}, \textbf{\texttt{bind(String name)}}
		Binds a \textit{module} of the specified name. The default name is: \texttt{"<module-name>/<module-name>"},
		where \texttt{<module-name>} is the name of the module.
