...
\newcommand\SetLanguageXX{
	...
	\lstset{
		morekeywords = {
			var, super, foreach, forever, init, release, and, or, not, null, import, module
		}
	}
}
...
\newcommand\SetLanguageXASM{
	...
	\lstset{
		morekeywords = {
			...
			CALL, INVK, JMP, JE, JNE, JG, JL, JGE, JLE, CMP, CMPF, RET, LDB, STB, LDW, STW, LDA, INC, DEC,
			NOT, ITF, FTI, $r0, $r1, $r2, $r3, $r4, $r5, $r6, $r7, $r8, $r9, $tr, $gp, $sp, $lb, $pc
		}
	}
}
...
\begin{center}
\begin{tabular}[ht]{
	| p{0.07 \textwidth} | p{0.1 \textwidth} | p{0.065 \textwidth} | p{0.375 \textwidth} | p{0.24 \textwidth} |
}
	\hline
...
\begin{center}
\begin{tabular}[ht]{
	| p{0.07 \textwidth} | p{0.1 \textwidth} | p{0.445 \textwidth} | p{0.26 \textwidth} |
}
	\hline
...
\begin{center}
\begin{tabular}[ht]{
	| p{0.07 \textwidth} | p{0.1 \textwidth} | p{0.135 \textwidth} | p{0.305 \textwidth} | p{0.24 \textwidth} |
}
	\hline
...
proc:
  ...
  ADD $r0, $r0, $r1 ; Calculate x := x + y
  PUSH $r0          ; Push result onto stack
  RET (1) 2         ; Return: result size = 1 word, argument size = 2 words
\end{lstlisting}
